...
基于 **Vue 3 + Spring Boot** 的全栈零售管理平台，支持多角色（管理员、商家、普通用户）权限管理，涵盖零售商品管理、订单处理、用户管理、数据统计等核心业务功能。
...
- 用户注册 / 登录（BCrypt 密码加密）
- 基于角色的访问控制（**RBAC**），三种角色：
  - **管理员 (Admin)** — 全部权限，可管理用户、零售商品、订单、分类、轮播图，查看数据大屏
  - **商家 (Merchant)** — 管理自己的零售商品、分类，处理订单
  - **普通用户 (User)** — 浏览零售商品、加入购物车、下单购买

### 🍪 零售商品管理
- 零售商品 CRUD（创建、查看、修改、删除/下架）
- 零售商品图片上传（支持多图）
- 商品分类管理（多级分类）
- 库存自动管理（库存归零自动下架）
...
| 商家 | `merchant001` | `123456` | 可管理零售商品和订单 |
| 普通用户 | `user001` | `123456` | 可浏览和购买零售商品 |
...
| 零售商品 | `/api/goods` | GET/POST | 零售商品列表 / 新增 |
| 零售商品 | `/api/goods/{id}` | GET/PUT/DELETE | 零售商品详情 / 修改 / 删除 |
